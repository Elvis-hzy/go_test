...
import (
	"errors"
	"fmt"
	body2 "github.com/Albert-Zhan/httpc/body"
	"log"
	"net/http"
	"strconv"
	"strings"
	...
	"github.com/Elvis-hzy/go_test/common"
	"github.com/Elvis-hzy/go_test/conf"
	"github.com/Elvis-hzy/go_test/service"
...
func (this *Seckill) SeckillInitInfo() (string, error) {
	...
	req.SetHeader("User-Agent", this.conf.Read("config", "DEFAULT_USER_AGENT"))
	req.SetHeader("Host", "marathon.jd.com")
	b := body2.NewUrlEncode()
	b.SetData("sku", skuId)
	b.SetData("num", seckillNum)
	b.SetData("isModifyAddress", "false")
	resp, body, err := req.SetUrl("https://marathon.jd.com/seckillnew/orderService/pc/init.action").SetBody(b).SetMethod("post").Send().End()
	if err != nil || resp.StatusCode != http.StatusOK {
...
func (this *Seckill) SubmitSeckillOrder() bool {
	...
	b := body2.NewUrlEncode()
	b.SetData("skuId", skuId)
	b.SetData("num", seckillNum)
	b.SetData("addressId", defaultAddress.Get("id").String())
	b.SetData("yuShou", "true")
	b.SetData("isModifyAddress", "false")
	b.SetData("name", defaultAddress.Get("name").String())
	b.SetData("provinceId", defaultAddress.Get("provinceId").String())
	b.SetData("cityId", defaultAddress.Get("cityId").String())
	b.SetData("countyId", defaultAddress.Get("countyId").String())
	b.SetData("townId", defaultAddress.Get("townId").String())
	b.SetData("addressDetail", defaultAddress.Get("addressDetail").String())
	b.SetData("mobile", defaultAddress.Get("mobile").String())
	b.SetData("mobileKey", defaultAddress.Get("mobileKey").String())
	b.SetData("email", defaultAddress.Get("email").String())
	b.SetData("postCode", "")
	b.SetData("invoiceTitle", invoiceTitle)
	b.SetData("invoiceCompanyName", "")
	b.SetData("invoiceContent", invoiceContentType)
	b.SetData("invoiceTaxpayerNO", "")
	b.SetData("invoiceEmail", "")
	b.SetData("invoicePhone", invoicePhone)
	b.SetData("invoicePhoneKey", invoicePhoneKey)
	b.SetData("invoice", invoiceInfo)
	b.SetData("password", paymentPwd)
	b.SetData("codTimeType", "3")
	b.SetData("paymentType", "4")
	b.SetData("areaCode", "")
	b.SetData("overseas", "0")
	b.SetData("phone", "")
	b.SetData("eid", eid)
	b.SetData("fp", fp)
	b.SetData("token", token)
	b.SetData("pru", "")
	resp, body, err := req.SetUrl("https://marathon.jd.com/seckillnew/orderService/pc/submitOrder.action?skuId=" + skuId).SetMethod("post").SetBody(b).Send().End()
	if err != nil || resp.StatusCode != http.StatusOK {
